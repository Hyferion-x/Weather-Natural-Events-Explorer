...
const formatKilowatts = (value: number | null | undefined) =>
  isFiniteNumber(value) ? `${(value / 1000).toFixed(2)} kW/m2` : 'N/A';

const getUVIndexCategory = (uvIndex: number | null | undefined) => {
  if (!isFiniteNumber(uvIndex)) return { level: 'Unavailable', color: 'text-gray-400', bgColor: 'bg-gray-900/20', borderColor: 'border-gray-900/30' };
...
const getUVProtectionAdvice = (uvIndex: number | null | undefined) => {
  if (!isFiniteNumber(uvIndex)) {
    return {
      protection: 'UV data unavailable',
      advice: 'Sun protection guidance is unavailable for this location.',
      recommendations: ['Check a local forecast before planning extended outdoor activity'],
    };
  }
  if (uvIndex <= 2) {
    return {
...
const SatelliteRadiationModal: React.FC<SatelliteRadiationModalProps> = ({
  ...
  const protectionAdvice = getUVProtectionAdvice(uvIndex);
      ...
      <div className="space-y-6">
        <div className={`${uvCategory.bgColor} p-4 rounded-lg border ${uvCategory.borderColor}`}>
          ...
          <div className="text-center mb-4">
            <div className={`text-4xl font-bold ${uvCategory.color}`}>{formatNumber(uvIndex)}</div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="text-white font-semibold">{formatNumber(current.uv_index_clear_sky)}</div>
            </div>
            <div>
        ...
        <div className="bg-blue-900/20 p-4 rounded-lg border border-blue-900/30">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="text-white font-semibold">{formatKilowatts(current.direct_normal_irradiance)}</div>
            </div>
            <div>
              <div className="text-gray-300">Diffuse</div>
              <div className="text-white font-semibold">{formatKilowatts(current.diffuse_radiation)}</div>
            </div>
            <div>
              <div className="text-gray-300">Terrestrial</div>
              <div className="text-white font-semibold">{formatKilowatts(current.terrestrial_radiation)}</div>
            </div>
            <div>
              <div className="text-gray-300">Shortwave</div>
              <div className="text-white font-semibold">{formatKilowatts(current.shortwave_radiation)}</div>
            </div>
            <div>
              <div className="text-gray-300">Global Tilted</div>
              <div className="text-white font-semibold">{formatKilowatts(current.global_tilted_irradiance)}</div>
            </div>
            <div>
              <div className="text-gray-300">Instant</div>
              <div className="text-white font-semibold">{formatKilowatts(current.shortwave_radiation_instant)}</div>
            </div>
          </div>
        </div>
        ...
        <div className={`${isFiniteNumber(uvIndex) && uvIndex > 7 ? 'bg-red-900/20 border-red-900/30' : 'bg-green-900/20 border-green-900/30'} p-4 rounded-lg border`}>
          <div className="flex items-center">
            {isFiniteNumber(uvIndex) && uvIndex > 7 ? (
            ...
            <div className="text-sm text-gray-300">
              <p className="font-semibold">
                {isFiniteNumber(uvIndex) && uvIndex > 7 ? 'High UV Alert' : 'UV Status'}
              </p>
              <p className="text-xs mt-1">
                {isFiniteNumber(uvIndex)
                  ? uvIndex > 7
                  ...
                  : 'UV status is unavailable for this location.'
                }
              </p>
            </div>
          </div>
