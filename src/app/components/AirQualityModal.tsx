...
const getAQICategory = (aqi: number | null | undefined) => {
  if (!isFiniteNumber(aqi)) {
    return { level: 'Unavailable', color: 'text-gray-400', bgColor: 'bg-gray-900/20', borderColor: 'border-gray-900/30' };
  }
...
const AirQualityModal: React.FC<AirQualityModalProps> = ({
  ...
  const maxAQI = Math.max(
    isFiniteNumber(europeanAQI) ? europeanAQI : -Infinity,
    isFiniteNumber(usAQI) ? usAQI : -Infinity
  );
  const primaryAQI = Number.isFinite(maxAQI) ? maxAQI : null;
      ...
      <div className="space-y-6">
        <div className={`${europeanCategory.bgColor} p-4 rounded-lg border ${europeanCategory.borderColor}`}>
          ...
          <div className="text-center mb-3">
            <div className={`text-4xl font-bold ${europeanCategory.color}`}>{formatAqi(europeanAQI)}</div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <div className="text-white font-semibold">{formatAqi(current.european_aqi_pm2_5)}</div>
            </div>
            <div>
              <div className="text-gray-300">PM10</div>
              <div className="text-white font-semibold">{formatAqi(current.european_aqi_pm10)}</div>
            </div>
            <div>
              <div className="text-gray-300">NO2</div>
              <div className="text-white font-semibold">{formatAqi(current.european_aqi_no2)}</div>
            </div>
            <div>
              <div className="text-gray-300">O3</div>
              <div className="text-white font-semibold">{formatAqi(current.european_aqi_o3)}</div>
            </div>
          </div>
        </div>
        ...
        <div className={`${usCategory.bgColor} p-4 rounded-lg border ${usCategory.borderColor}`}>
          ...
          <div className="text-center mb-3">
            <div className={`text-4xl font-bold ${usCategory.color}`}>{formatAqi(usAQI)}</div>
            <div className={`text-lg font-semibold ${usCategory.color}`}>{usCategory.level}</div>
          </div>
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <div className="text-gray-300">PM2.5</div>
              <div className="text-white font-semibold">{formatAqi(current.us_aqi_pm2_5)}</div>
            </div>
            <div>
              <div className="text-gray-300">PM10</div>
              <div className="text-white font-semibold">{formatAqi(current.us_aqi_pm10)}</div>
            </div>
            <div>
              <div className="text-gray-300">NO2</div>
              <div className="text-white font-semibold">{formatAqi(current.us_aqi_no2)}</div>
            </div>
            <div>
              <div className="text-gray-300">O3</div>
              <div className="text-white font-semibold">{formatAqi(current.us_aqi_o3)}</div>
            </div>
            <div>
              <div className="text-gray-300">SO2</div>
              <div className="text-white font-semibold">{formatAqi(current.us_aqi_so2)}</div>
            </div>
            <div>
              <div className="text-gray-300">CO</div>
              <div className="text-white font-semibold">{formatAqi(current.us_aqi_co)}</div>
            </div>
          </div>
        </div>
        ...
        <div className="bg-blue-900/20 p-4 rounded-lg border border-blue-900/30">
          ...
          <div className="text-sm text-gray-300 space-y-2">
            <p>{getHealthAdvice(primaryAQI)}</p>
            ...
            {isFiniteNumber(primaryAQI) && primaryAQI <= 50 && (
